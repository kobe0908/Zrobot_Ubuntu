...
//    usleep(2000 * 100);
    ...
    printf("initial in the function is okay! \n");
}
...
void move(int act,long spd)
{
	...
	printf("the switch module is okay! ");
	car_set();
}
// out function
void stop()   //the out function
{
	left_wheel(0);
	right_wheel(0);
}
//out function
void position_ctrl(int wheelL_set, int wheelR_set, int wheelL_speed, int wheelR_speed)   //the out function
{
	cur_posL += get_left();
	cur_posR += get_right();
