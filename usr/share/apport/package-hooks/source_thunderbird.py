'''thunderbird apport hook

/usr/share/apport/package-hooks/thunderbird.py
...
class Plugin(object):
    ...
    def dump(self):
        if self.path.startswith(os.path.join(os.environ['HOME'], '.thunderbird')):
            location = "[Profile]"
        else:
...
class Prefs:
    ...
    def __init__(self, profile_path, extra_paths=None, whitelist=None, blacklist=None):
        ...
        if profile_path != None:
            locations = [
                "/usr/lib/thunderbird/omni.ja:greprefs.js",
                "/usr/lib/thunderbird/omni.ja:defaults/pref/*.js",
                "/usr/lib/thunderbird/defaults/pref/*.js",
                "/usr/lib/thunderbird/defaults/pref/unix.js",
                "/usr/lib/thunderbird/omni.ja:defaults/preferences/*.js"
                "/usr/lib/thunderbird/defaults/preferences/*.js"
            ]

            append_dirs = [ 'defaults/preferences/*.js' ]
            if os.path.isdir('/usr/lib/thunderbird/distribution/bundles'):
                bundles = os.listdir('/usr/lib/thunderbird/distribution/bundles')
                bundles.sort(reverse=True)
                for d in append_dirs:
                    for bundle in bundles:
                        path = os.path.join('/usr/lib/thunderbird/distribution/bundles', bundle)
...
class Extension:
    ...
    def active_but_incompatible(self):
        # FIXME: Assumes that extensions.strictCompatibility is always false
...
class Profile:
    ...
    def _populate_extensions(self):
        # We copy the db as it's locked whilst Thunderbird is open. This is still racy
...
def add_info(report, ui):
    ...
    ddproc = Popen(['dpkg-divert', '--truename', '/usr/bin/thunderbird'], stdout=subprocess.PIPE, stderr=subprocess.STDOUT, universal_newlines=True)
    truename = ddproc.communicate()
    if ddproc.returncode == 0 and truename[0].strip() != '/usr/bin/thunderbird':
        ddproc = Popen(['dpkg-divert', '--listpackage', '/usr/bin/thunderbird'], stdout=subprocess.PIPE, stderr=subprocess.STDOUT, universal_newlines=True)
        diverter = ddproc.communicate()
        report['UnreportableReason'] = "/usr/bin/thunderbird has been diverted by a third party package (%s)" % diverter[0].strip()
        return

    conf_dir = os.path.join(os.environ["HOME"], ".thunderbird")
    appini = AppINIParser('/usr/lib/thunderbird')
    ...
    for profile in profiles:
        if profile.running and not profile.current:
            report["UnreportableReason"] = "Thunderbird has been upgraded since you started it. Please restart all instances of Thunderbird and try again"
            return
    ...
    if 'thunderbird' == 'thunderbird-trunk':
        report["Channel"] = 'nightly'
        append_tag('nightly-channel')
        if report["SourcePackage"] == 'thunderbird-trunk':
            report["SourcePackage"] = 'thunderbird'
    else:
        channelpref = Prefs(None, ['/usr/lib/thunderbird/defaults/pref/channel-prefs.js'], whitelist = [ r'app\.update\.channel' ])
    ...
    # XXX: Apparmor stuff disabled for Thunderbird
    ...
    #        if f.startswith("usr.bin.thunderbird"):
    #            add_apparmor = False
    #            break
    #if add_apparmor:
