...
def check_is_supported(report, ui=None):
    ...
    if status == "Active Development":
        ...
        response = ui.choice(
            "'%s' is no longer under development, but technical support is still \n"
            ...
            "through technical support channels first." %(distro_codename),
            [
        ...
        # Referred by technical support
            ...
            report['Tags'] += ' ' + 'referred-by-support'
        ...
        # Just want to file a bug report
        elif 3 in response:
            return True

        # Anything else should be redirected to technical support channels
        else:
            ui.information("http://www.ubuntu.com/support/ is the best place to get free help with technical issues.")
...
def attach_nvidia_info(report, ui=None):
    ...
    if os.path.exists('/usr/bin/nvidia-bug-report.sh'):
        if ui.yesno("Would you like to generate and attach an NVIDIA bug reporting file? (~/nvidia-bug-report.log.gz)") == True:
            attach_root_command_outputs(report, {
...
def attach_debugging_interest_level(report, ui):
    ...
    if (report.get('SourcePackage','Unknown') in core_x_packages or
        ...
        choices = [
            ...
            "Yes,",
            "Yes, even including gdb or git bisection work if needed",
            ]
        response = ui.choice(text, choices)
        if response == None:
            raise StopIteration
