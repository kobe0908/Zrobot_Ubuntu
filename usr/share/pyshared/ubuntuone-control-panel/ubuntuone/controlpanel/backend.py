...
DOWNLOAD_KEY = u'max_download_speed'

FILE_SYNC_DISABLED = u'file-sync-disabled'
FILE_SYNC_DISCONNECTED = u'file-sync-disconnected'
FILE_SYNC_ERROR = u'file-sync-error'
FILE_SYNC_IDLE = u'file-sync-idle'
FILE_SYNC_STARTING = u'file-sync-starting'
FILE_SYNC_STOPPED = u'file-sync-stopped'
FILE_SYNC_SYNCING = u'file-sync-syncing'
FILE_SYNC_UNKNOWN = u'file-sync-unknown'
...
UBUNTUONE_FROM_OAUTH = u'https://one.ubuntu.com/api/1.0/from_oauth/'
...
class ControlBackend(object):
    ...
    def get_home_dir(self):
        """Return a defer with the home_dir from ubuntu one client."""
        return self.sd_client.get_home_dir()
    ...
    def _process_file_sync_status(self, status):
        ...
        * MSG_KEY: a non translatable but human readable string of the status.
        ...
        Return None if we receive and invalid type in status.
    ...
    def _process_path(self, path):
        ...
        home_dir = yield self.get_home_dir()
        if not home_dir.endswith(os.path.sep):
            home_dir += os.path.sep
    ...
    def build_signed_iri(self, iri):
        """Returned an OAuth signed iri."""
        credentials = None
        if iri.startswith(UBUNTUONE_LINK):
            credentials = yield self.get_credentials()

        if credentials:
            parameters = {'next': iri}
            iri = yield self.wc.build_signed_iri(UBUNTUONE_FROM_OAUTH,
                                                 parameters)
        returnValue(iri)
    ...
    def login(self, email=None, password=None):
        """Login using 'email' and 'password' if not None, else prompt user."""
        result = None
        if email is not None and password is not None:
            result = yield self.login_client.login_email_password(
                        email=email, password=password)
        else:
            result = yield self.login_client.login()
        # cache credentils
    ...
    def register(self):
        """Register a new user."""
        result = yield self.login_client.register()
        # cache credentils
        self._credentials = result
        returnValue(result)
    ...
    @log_call(logger.debug)
    @inlineCallbacks
    def volumes_info(self, with_storage_info=True, refresh=False):
        ...
        if refresh:
            yield self.sd_client.refresh_volumes()
        ...
        display_name = yield self._process_path(root_dir)
        ...
        for folder in folders:
            ...
            display_name = yield self._process_path(folder[u'path'])
    ...
    def validate_path_for_folder(self, folder_path):
        ...
        return self.sd_client.validate_path(folder_path)
    ...
    def file_sync_settings_info(self):
        ...
        limits = yield self.sd_client.get_throttling_limits()
        limits_enabled = yield self.sd_client.bandwidth_throttling_enabled()
        if limits_enabled:
            result[DOWNLOAD_KEY] = limits['download']
            result[UPLOAD_KEY] = limits['upload']
        else:
            result[DOWNLOAD_KEY] = -1
            result[UPLOAD_KEY] = -1
